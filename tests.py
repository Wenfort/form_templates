...
def second_test_case():
    """
    Второй тестовый случай тоже отправляет правильный запрос, но содержит дополнительное поле, которое не находится
        в первом шаблоне в БД. Ожидается, что в качестве ответа должно быть получено название шаблона
...
def sixth_test_case():
    """
    В шестом тестовом случае проверяется корректная работа альтернативного формата ввода времени.
...
def seventh_test_case():
    ...
    Из-за не указанной причины возврата денег, ни один из шаблонов не подходит.
    """
    test_post_data = {
        'user_name': 'I am an angry user!',
        'user_order_date': '2020-02-10',
